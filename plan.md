...
## Coherence pass + lecture titles (2026-09-21, branch `main`) — closed
...
timetable. Committed and pushed to `origin/main` at the user's explicit
request. **No next step queued** — this was a complete, closed batch;
only reopen it if the user raises something new.

Note: this work was first committed on a `atmosphere-effects` branch
that turned out to be shared with a second, unrelated Claude session
doing an actual "atmosphere effects" (in-fiction annotation) demo for the
user — same branch name by coincidence, not related work. That first
commit was made with `git add -A` and accidentally swept up several of
that session's uncommitted files (its in-fiction record-annotation CSS
classes and matching markup) alongside this batch's own changes. The
user caught it and asked for a clean move to `main`: done via
`git cherry-pick -n` plus `git restore` for the files that were entirely
the other session's (`TimetableGrid.astro`, `case-file.md`, both people
bios, `06-case-clinic-2.md`, `11-fourth-dimensional-hypothesis.md`,
`incidents/index.mdx`, `people/[slug].astro`, `sessions/[slug].astro`,
`timetable.astro`), and manual hunk-splitting for the two files that
genuinely mixed both sessions' changes in the same file (`week-06.md`'s
body, `global.css`). Verified, by name, zero trace of any of that other
session's markers in the final staged diff before committing to `main`.
The original `atmosphere-effects` branch and its commits were left
untouched — this was a copy of the relevant content over, not a merge or
rewrite of that branch's history.
